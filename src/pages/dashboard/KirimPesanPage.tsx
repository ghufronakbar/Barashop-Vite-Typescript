...
import { KirimPesan } from "@/model/KirimPesan";
...
interface KirimPesanDTO {
  id: string;
  subjek: string;
  pesan: string;
}

const initKirimPesanDTO: KirimPesanDTO = {
  id: "",
  subjek: "",
  pesan: "",
};
...
const useKirimPesans = () => {
  const [data, setData] = useState<KirimPesan[]>([]);
  const [search, setSearch] = useState("");
  const [form, setForm] = useState<KirimPesanDTO>(initKirimPesanDTO);
  ...
  const fetchData = async () => {
    try {
      const res = await api.get<Api<KirimPesan[]>>("/pesan");
  ...
  const onClickTemplate = (item: (typeof TEMPLATES)[number]) => {
    setForm({
      id: "",
      subjek: item.subjek,
      pesan: item.pesan,
